...
if (!isset($_GET['patient_id'])) { echo "❌ No patient selected"; exit; }
$patient_id = intval($_GET['patient_id']);
$pname = $conn->query("SELECT name FROM users WHERE id='$patient_id'")->fetch_assoc()['name'] ?? 'Patient';
...
if ($check && $check->num_rows > 0) { $checklist_id = $check->fetch_assoc()['id']; $stmt2 = $conn->prepare("SELECT * FROM checklist_items WHERE checklist_id = ?"); $stmt2->bind_param("i", $checklist_id); $stmt2->execute(); $result = $stmt2->get_result(); }
else { $result = null; }
?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    <div class="page-header"><h1>📋 <?php echo htmlspecialchars($pname); ?>'s Medicines</h1><p>Medicine adherence checklist for this patient.</p></div>
    <a href="monitor_view.php" class="btn btn-secondary btn-sm" style="margin-bottom:20px;">← Back</a>
    <div class="card">
    ...
    <div class="table-wrap"><table>
        ...
        <?php $scheduled_time = $row['times_of_day'] ?? $row['due_time'] ?? 'Not set'; ?>
        <tr>
            <td><strong><?php echo htmlspecialchars($row['medicine_name']); ?></strong></td>
            <td><?php echo htmlspecialchars($scheduled_time); ?></td>
    ...
    <?php else: ?><div class="empty-state"><div class="empty-icon">💊</div><h3>No medicines found</h3><p>This patient hasn't added any medicines yet.</p></div><?php endif; ?>
    </div>
</main></div></body></html>
<script>
// Theme Toggle Logic
